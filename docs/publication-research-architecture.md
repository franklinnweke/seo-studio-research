...
- Latest completed research commit before this amendment: `79f1021` (`Record fixed writer compatibility evidence`).
...
- The full 20-item compatibility matrix and fixed-writer compatibility pass are complete with sanitized evidence under `evaluation/results/`. A separate two-model Qwen3.6 amendment is preflight-ready under the unchanged contract; its live execution, protocol freeze, the approximately 120-item final dataset, full experiments, human annotation, statistical analysis, and the manuscript remain outstanding.
...
- A5 evidence explicitly forbids quality ranking from the one-image smoke, completed five-condition compatibility matrix, and five-call writer pass. A separate Qwen3.6 amendment is preflight-ready, but its live outcomes must be accounted before Gate 3 can close.
...
- `evaluation/configs/candidate-amendment-20260717.json` is the amendment decision record. It preserves the original matrix as immutable evidence, documents the selection rule and unchanged controls, and records that supervisor acknowledgement remains required before final protocol freeze even though execution is authorized within the approved project-use rights.
...
Status on July 17, 2026: substantially implemented, Gate 3 still open. The CLI now covers preflight, strict schema execution, digest-verified warm compatibility execution with append-only resume and an absolute request deadline, validation, recovery-aware accounting, multi-block normalization, blinding, generated non-ranking pilot reporting, and the frozen text-only writer check. The licensed 20-image matrix is complete: only Gemma 3 12B met the frozen 95% gate, so no quality winner was selected and the required two-challenger advancement set cannot yet be formed. The pinned Qwen3.5 writer passed all five deterministic source-fact conditions. Pilot evidence projects approximately 17.4 active inference hours for a 120-item five-model screen and a three-condition single-rated-repeat package of 360 outputs; the two-reviewer burden is 24/36/60 hours at explicit 2/3/5-minute planning scenarios and still requires observed calibration. A separate, unchanged-contract amendment for two already-installed Qwen3.6 variants is now documented and preflight-ready. Remaining A5 work is live amendment execution/accounting, qualification of at least one additional challenger, and observed reviewer-time calibration.
...
- [ ] Execute and account the amendment; qualify at least one additional challenger before comparative quality inspection.
...
7. Execute the preflight-ready Qwen3.6 amendment under the unchanged compatibility contract, account every attempt, and obtain supervisor acknowledgement before final protocol freeze; do not inspect comparative quality first.
