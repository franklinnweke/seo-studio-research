...
- The full 20-item compatibility matrix, fixed-writer pass, separate two-model Qwen3.6 amendment, and source-linked 15-call Protocol 2.2 truncation-repair stage are complete with sanitized evidence under `evaluation/results/`. Qwen3.5 9B and Gemma 3 12B meet the unchanged 95% pipeline-validity gate; the Qwen2.5-VL 3B baseline remains a below-gate reference. A pre-freeze catalog correction paused quality screening and selected the same-size Gemma 4 12B instruction-tuned Q4_K_M package because Gemma 4 was available before the original candidate set was finalized. Its shared-runtime pull was rejected at manifest resolution with HTTP 412 without installing a layer or changing existing resources. The project team has now authorized a project-owned newer runtime, separate model store, and loopback-only listener for one 20-image block each from the baseline, Qwen3.5, and Gemma 4. These 60 outputs will supply both compatibility evidence and the later blinded pilot screen; they will not be generated twice. No quality winner has been selected. Isolated-runtime execution, supervisor acknowledgement, blinded quality screening, reviewer-time calibration, final protocol freeze, the approximately 120-item dataset, full experiments, human annotation, statistical analysis, and manuscript remain outstanding.
...
- The predeclared Gemma 4 pull stopped at registry manifest resolution with HTTP 412 because the shared Ollama 0.24.0 runtime does not support the package. The post-attempt `$davneet-dgx-access` check confirmed the service remained healthy, no model was loaded, Gemma 4 was not installed, and existing models were unchanged. Do not silently upgrade the shared service. The next decision is either a separately authorized project-isolated newer runtime/model store or a transparent runtime-incompatible exclusion that narrows the paper's claim to the deployed Ollama 0.24.0 stack.
- The project team selected the isolated-runtime option. It authorizes only a project-owned current Ollama runtime, separate model store, loopback listener, and the baseline/Qwen3.5/Gemma 4 packages. It does not authorize a shared-service restart or upgrade, existing-model-store mutation, owner-file changes, duplicate quality generation, or comparative inspection before blinding. The isolated 60-output pilot becomes the sole compatibility and quality-screening source for those three conditions.
...
- [ ] Install and digest-lock the isolated runtime and three packages, preflight them, then collect one 20-image block per condition for compatibility and later blinded quality screening.
