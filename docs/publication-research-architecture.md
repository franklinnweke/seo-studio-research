...
- The full 20-item compatibility matrix, fixed-writer pass, separate two-model Qwen3.6 amendment, and source-linked 15-call Protocol 2.2 truncation-repair stage are complete with sanitized evidence under `evaluation/results/`. Qwen3.5 9B and Gemma 3 12B meet the unchanged 95% pipeline-validity gate; the Qwen2.5-VL 3B baseline remains a below-gate reference. A pre-freeze catalog correction paused quality screening and selected the same-size Gemma 4 12B instruction-tuned Q4_K_M package because Gemma 4 was available before the original candidate set was finalized. Its shared-runtime pull was rejected at manifest resolution with HTTP 412 without installing a layer or changing existing resources. The project team then installed an official project-owned Ollama 0.32.1 ARM64 runtime, separate model store, and loopback-only listener and digest-locked the baseline, Qwen3.5, and Gemma 4 packages without changing the shared service or owner files. The 8192-context, 60-output isolated one-shot matrix is complete and supplies both compatibility evidence and the later blinded pilot screen. Four source-linked truncation repairs are frozen but not yet executed. No quality winner has been selected. Isolated Protocol 2.2 repair, supervisor acknowledgement, blinded quality screening, reviewer-time calibration, final protocol freeze, the approximately 120-item dataset, full experiments, human annotation, statistical analysis, and manuscript remain outstanding.
...
- [ ] Execute and account the four isolated Protocol 2.2 repairs, then fix the current-generation compatibility decision without lowering the 95% gate.
