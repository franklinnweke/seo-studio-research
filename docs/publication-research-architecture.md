...
- The full 20-item compatibility matrix, fixed-writer pass, separate two-model Qwen3.6 amendment, and source-linked 15-call Protocol 2.2 truncation-repair stage are complete with sanitized evidence under `evaluation/results/`. Qwen3.5 9B and Gemma 3 12B meet the unchanged 95% pipeline-validity gate; the Qwen2.5-VL 3B baseline remains a below-gate reference. A pre-freeze catalog correction paused quality screening and selected the same-size Gemma 4 12B instruction-tuned Q4_K_M package because Gemma 4 was available before the original candidate set was finalized. Its shared-runtime pull was rejected at manifest resolution with HTTP 412 without installing a layer or changing existing resources. The project team has now installed an official project-owned Ollama 0.32.1 ARM64 runtime, separate model store, and loopback-only listener, then digest-locked the baseline, Qwen3.5, and Gemma 4 packages without changing the shared service or owner files. The isolated block freezes an 8192-token context window across all three conditions. Its 60 outputs will supply both compatibility evidence and the later blinded pilot screen; they will not be generated twice. No quality winner has been selected. Measured isolated-runtime execution, supervisor acknowledgement, blinded quality screening, reviewer-time calibration, final protocol freeze, the approximately 120-item dataset, full experiments, human annotation, statistical analysis, and manuscript remain outstanding.
...
- A5 evidence explicitly forbids quality ranking from the one-image smoke, completed five-condition compatibility matrix, five-call writer pass, completed Qwen3.6 amendment, and completed Protocol 2.2 repair stage. Gate 3 remains open pending supervisor acknowledgement, release of the blinded three-condition quality-screening package, and observed reviewer-time calibration before final protocol freeze.
...
- The project team selected the isolated-runtime option. The official Ollama 0.32.1 ARM64 archive and separate model store are now installed in a project-owned location with a loopback-only listener; all three packages are digest-locked in `evaluation/configs/models-isolated-gemma4.toml`, with sanitized installation evidence in `evaluation/configs/isolated-runtime-evidence-20260717.json`. Qwen2.5-VL 3B and Qwen3.5 retain exactly the same digests as the original shared-runtime conditions. This authorization does not include a shared-service restart or upgrade, existing-model-store mutation, owner-file changes, duplicate quality generation, or comparative inspection before blinding. The isolated 60-output pilot becomes the sole compatibility and quality-screening source for those three conditions and fixes `num_ctx=8192` for internal comparability.
...
- the additive Gemma 4 pull was rejected with HTTP 412 before any model layer was installed because it requires a newer Ollama runtime. The shared 0.24.0 service was not changed. Gemma 4 is therefore a documented runtime-incompatible candidate unless a project-isolated newer runtime is separately authorized.
...
Status on July 17, 2026: substantially implemented, Gate 3 ready for closure evidence but still open. The original five-model matrix, fixed writer check, separate two-model Qwen3.6 amendment, and Protocol 2.2 truncation-repair stage are complete and accounted. All 15 repairs were collected; 6 were valid and 9 remained invalid, with no transport failures. Qwen3.5 and Gemma each reached 19/20 pipeline validity and are the two eligible challengers. The below-gate Qwen2.5-VL 3B baseline is retained only as a reference, forming a three-condition blinded quality-screening set; no quality winner has been selected. The repair stage consumed 0.536 analyzed inference hours. The earlier five-model planning scenario projected approximately 17.4 active inference hours, but the full-study runtime must be recalculated after the quality screen fixes the final architecture conditions. Reviewer burden remains a planning estimate and needs observed calibration. Remaining A5 work is supervisor acknowledgement, blinded-package generation and leakage validation for the three-condition set, observed reviewer-time calibration, and final protocol freeze.
...
- [ ] Reconfirm live isolated-runtime health and locked digests, then collect one 20-image block per condition for compatibility and later blinded quality screening.
- [ ] Obtain supervisor acknowledgement, generate and leakage-check the blinded three-condition quality-screening package, and calibrate observed reviewer time before final protocol freeze.
...
7. Treat the Qwen3.6 amendment and the 15-call Protocol 2.2 repair stage as complete; verify their committed sanitized reports, but do not rerun them unless a formally recorded protocol revision requires it. The project-owned Ollama 0.32.1 runtime and separate model store now contain digest-locked Qwen2.5-VL 3B, Qwen3.5, and Gemma 4 packages. Reconfirm them through `$davneet-dgx-access`; never upgrade or restart the shared service, mutate its model store, or alter owner files. Preserve Gemma 3 as legacy deployed-stack evidence and do not lower the 95% gate.
8. Collect the prospectively frozen isolated three-condition block using `evaluation/configs/pilot-isolated-gemma4.toml` and `evaluation/configs/compatibility-isolated-gemma4-criteria.toml`. Do not inspect comparative quality. If any 384-token outcome is invalid solely because `done_reason=length`, freeze a source-linked Protocol 2.2 repair plan before making the one permitted 768-token recovery.
9. After the isolated compatibility decision fixes the current-generation challenger set, obtain supervisor acknowledgement, then normalize the baseline/Qwen3.5/Gemma 4 conditions, complete accounting, generate the three-condition blinded quality-screening package, validate it for identity leakage, and keep its mapping private. Do not inspect comparative quality before acknowledgement and blinding.
10. Use the generated runtime, storage, claim-count, and reviewer-burden scenarios for planning, then collect observed reviewer calibration time before protocol freeze and recalculate the full-study runtime after the quality screen fixes the final architecture conditions; never use pilot quality effects to choose a significance-friendly sample size.
