...
- Latest completed research commit before the completed live amendment: `9f89d7b` (`Prepare controlled Qwen3.6 candidate amendment`).
...
- The full 20-item compatibility matrix, fixed-writer pass, and separate two-model Qwen3.6 amendment are complete with sanitized evidence under `evaluation/results/`. Neither amendment candidate met the unchanged 95% gate. A source-linked 15-call Protocol 2.2 truncation-repair plan is preflight-ready; its live execution, final protocol freeze, the approximately 120-item dataset, full experiments, human annotation, statistical analysis, and manuscript remain outstanding.
...
- A5 evidence explicitly forbids quality ranking from the one-image smoke, completed five-condition compatibility matrix, five-call writer pass, and completed Qwen3.6 amendment. Gate 3 remains open pending the separately frozen Protocol 2.2 truncation-repair evidence and supervisor acknowledgement before final protocol freeze.
...
- Because only Gemma met the original one-shot 95% gate, the predeclared requirement for two eligible non-baseline challengers was unsatisfied. The completed Qwen3.6 amendment did not resolve that deficit. Do not lower the threshold or advance ineligible models after seeing the results; Gate 3 now remains open pending the separately frozen Protocol 2.2 truncation-repair evidence and supervisor acknowledgement.
...
Status on July 17, 2026: substantially implemented, Gate 3 still open. The original five-model matrix, fixed writer check, and separate two-model Qwen3.6 amendment are complete and accounted. Only Gemma met the one-shot 95% gate; dense Qwen3.6 reached 15/20 and Qwen3.6 MoE 13/20, so no quality winner was selected. Failure taxonomy showed that all schema-invalid results were 384-token truncations, while the remaining failures were final timeouts. The harness now includes a preflighted Protocol 2.2 stage for exactly 15 explicit, source-linked 768-token truncation repairs under the unchanged 240-second deadline and 95% pipeline-validity gate. Pilot evidence projects approximately 17.4 active inference hours for the original 120-item five-model screen; reviewer burden remains 24/36/60 hours at 2/3/5-minute planning assumptions and needs observed calibration. Remaining A5 work is live truncation-repair execution/accounting, supervisor acknowledgement and protocol freeze, finalist formation if the pipeline gate is met, and observed reviewer-time calibration.
...
- [ ] Execute and account the Protocol 2.2 truncation repairs; form the advancement set only if the unchanged 95% pipeline-validity gate is met.
...
7. Treat the Qwen3.6 amendment as complete and failed at the one-shot gate. Execute only the frozen 15-call Protocol 2.2 truncation-repair plan, account its source-linked outcomes separately, and obtain supervisor acknowledgement before final protocol freeze; do not retry timeouts, lower 95%, or inspect comparative quality first.
