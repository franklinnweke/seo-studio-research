...
- The full 20-item compatibility matrix, fixed-writer pass, separate two-model Qwen3.6 amendment, and source-linked 15-call Protocol 2.2 truncation-repair stage are complete with sanitized evidence under `evaluation/results/`. Qwen3.5 9B and Gemma 3 12B meet the unchanged 95% pipeline-validity gate; the Qwen2.5-VL 3B baseline remains a below-gate reference. No quality winner has been selected. Supervisor acknowledgement, the blinded three-condition quality screen, reviewer-time calibration, final protocol freeze, the approximately 120-item dataset, full experiments, human annotation, statistical analysis, and manuscript remain outstanding.
...
7. Treat the Qwen3.6 amendment and the 15-call Protocol 2.2 repair stage as complete; verify their committed sanitized reports, but do not rerun them unless a formally recorded protocol revision requires it. Preserve one-shot and pipeline outcomes separately and do not lower the 95% gate.
8. Obtain supervisor acknowledgement, then normalize the selected baseline/Qwen3.5/Gemma conditions, complete accounting, generate the three-condition blinded quality-screening package, validate it for identity leakage, and keep its mapping private. Do not inspect comparative quality before acknowledgement and blinding.
