...
def test_protocol_audit_command_reports_frozen_protocol_ready(tmp_path: Path, capsys) -> None:
    evaluation_root = Path(__file__).resolve().parents[1]
    ...
    exit_code = main(
        [
            "protocol-audit",
            "--protocol",
            str(evaluation_root / "configs" / "full-study-protocol-v1.json"),
            "--output",
            str(output_path),
        ]
    )
    ...
    assert exit_code == 0
    assert output["status"] == "freeze_ready"
