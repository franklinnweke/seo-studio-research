...
python -m seo_studio_eval protocol-audit --protocol configs/full-study-protocol-v1.json --output results/full-study-protocol-audit-v1.json
python -m seo_studio_eval full-study-plan --protocol configs/full-study-protocol-v1.json --config configs/full-study.toml --output configs/full-study-execution-plan-v1.jsonl --summary results/full-study-execution-plan-v1.json
python -m seo_studio_eval full-study-plan-validate --protocol configs/full-study-protocol-v1.json --config configs/full-study.toml --plan configs/full-study-execution-plan-v1.jsonl --output results/full-study-execution-plan-validation-v1.json
python -m seo_studio_eval full-study-collect --protocol configs/full-study-protocol-v1.json --config configs/full-study.toml --plan configs/full-study-execution-plan-v1.jsonl --phase <primary_generation|decomposed_writer|context_ablation> --base-url http://127.0.0.1:<local-tunnel-port> --run-dir runs/<full-study-run-id> --run-id <full-study-run-id> --system-snapshot-ref <private-snapshot-reference> --max-new-attempts <segment-size>
python -m seo_studio_eval sample-size-plan --protocol configs/full-study-protocol-v1.json --output results/full-study-sample-size-sensitivity-v1.json
...
`protocol-audit` validates the frozen Gate 4 contract, verifies pinned prompt and execution-plan hashes, recomputes stage-cell and human-review accounting, and lists every scientific or execution blocker. When the full-study manifest exists, the audit parses its schema, verifies all referenced files and hashes, and checks its item count, split, and exact domain allocation. The selected 128-image design yields 3,012 planned stage cells, 876 unique review items, and 1,071 reviewer assignments. Listener binding remains recorded in infrastructure evidence but is not a scientific freeze criterion; live sessions use the approved SSH tunnel. The audit performs no live DGX access.
The latest generated public audit is `results/full-study-protocol-audit-v1.json`; regenerate it whenever the frozen contract is amended.
`sample-size-plan` uses only the approved pre-data meaningful effects and declared nuisance-parameter sensitivity grid. It does not inspect comparative model quality. Its `decision_recorded` result links the pre-data sensitivity evidence to the practical estimation-first choice in `configs/full-study-sample-size-decision-20260719.json`.

`full-study-plan` is offline and deterministic. The pinned plan contains 3,012 expected stage cells: 1,536 primary-generation cells, 1,152 fixed-writer cells, and 324 context-ablation cells. The context cells deliberately retain a `selected_vision_model` dependency until the frozen selection rule has been applied; the collector requires an immutable selection record before that phase. A failed upstream facts cell remains an explicit failed downstream stage cell and suppresses the corresponding writer request, so 3,012 is the complete planned outcome population and the maximum number of inference requests, not a promise that every downstream API request will be sent.

`full-study-collect` refuses a draft or invalid protocol before inference. It verifies the plan hash, free storage, clean worktree, live model digests, run identity, and existing record-to-cell bindings. Records are append-only. A completed outcome is never regenerated; one visible transport recovery may supersede a recorded transport interruption, while timeout, schema, safety, and quality failures are final. `--max-new-attempts` creates a deliberate segment boundary and unloads the active model at a block or segment boundary.
...
The isolated one-shot matrix completed with 60 analyzed outcomes and 62 raw records: Qwen2.5-VL 3B produced 20/20 schema-valid outputs, Qwen3.5 produced 18/20, and Gemma 4 produced 17/20. Two tunnel failures were superseded by explicitly linked bounded recoveries. Failure-metadata review—without comparative claim inspection—found four `done_reason=length` outcomes at the frozen 384-token cap (Qwen3.5: one; Gemma 4: three) and one final Qwen3.5 inference timeout, which was not repairable. `results/truncation-repair-isolated-gemma4-plan-20260718.json` froze exactly those four Protocol 2.2 repairs before any 768-token call.

The completed isolated repair report is `results/truncation-repair-isolated-gemma4-20260718.json` and `.md`. Qwen3.5 repaired 1/1 and reached 19/20 pipeline validity, passing the unchanged 95% gate. Gemma 4 repaired only 1/3 and reached 18/20, so it failed the gate and does not advance. The protocol reassessment therefore retains the original same-runtime screening set—Qwen2.5-VL 3B as the reference plus eligible Qwen3.5 and Gemma 3—as a deployed SEO Studio stack comparison. Gemma 3 is defensible in that narrow role because it was prospectively selected from the deployed inventory and passed the unchanged gate; it must not be described as current, newest, globally best, or representative of every open multimodal model. Gemma 4 is reported as a prospectively tested current-generation exclusion. Identity-safe blinding and calibration are complete; the private supervisor evidence record and explicit full-study protocol freeze remain outstanding.

The final calibration evidence is `results/recal-analysis-20260719.json` with the human-readable report at `results/recal_analysis_results.md`. The earlier `calibration-analysis-20260719.json` and `calibration_analysis_results.md` preserve the first diagnostic pass and must not be substituted for the final recalibration result.
