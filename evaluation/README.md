...
python -m seo_studio_eval full-study-plan --protocol configs/full-study-protocol-v1.draft.json --config configs/full-study.toml --output configs/full-study-execution-plan-v1.jsonl --summary results/full-study-execution-plan-v1.json
python -m seo_studio_eval full-study-plan-validate --protocol configs/full-study-protocol-v1.draft.json --config configs/full-study.toml --plan configs/full-study-execution-plan-v1.jsonl --output results/full-study-execution-plan-validation-v1.json
python -m seo_studio_eval full-study-collect --protocol configs/full-study-protocol-v1.draft.json --config configs/full-study.toml --plan configs/full-study-execution-plan-v1.jsonl --phase <primary_generation|decomposed_writer|context_ablation> --base-url http://127.0.0.1:<local-tunnel-port> --run-dir runs/<full-study-run-id> --run-id <full-study-run-id> --system-snapshot-ref <private-snapshot-reference> --max-new-attempts <segment-size>
...
`protocol-audit` validates the Gate 4 draft, verifies pinned prompt and execution-plan hashes, recomputes stage-cell and human-review accounting, and lists every unresolved freeze blocker. When the full-study manifest exists, the audit parses its schema, verifies all referenced files and hashes, and checks its item count, split, and exact domain allocation. The selected 128-image design yields 3,012 planned stage cells, 876 unique review items, and 1,071 reviewer assignments. A `draft_blocked` result remains correct until listener security and backup evidence are complete and the protocol status is deliberately changed. It performs no live DGX access and must never be treated as execution authorization.
