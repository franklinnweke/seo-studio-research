...
`protocol-audit` validates the Gate 4 draft, verifies pinned prompt and execution-plan hashes, recomputes stage-cell and human-review accounting, and lists every unresolved freeze blocker. When the full-study manifest exists, the audit parses its schema, verifies all referenced files and hashes, and checks its item count, split, and exact domain allocation. The selected 128-image design yields 3,012 planned stage cells, 876 unique review items, and 1,071 reviewer assignments. A `draft_blocked` result remains correct until listener security is complete and the protocol status is deliberately changed. It performs no live DGX access and must never be treated as execution authorization.
